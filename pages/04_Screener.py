...
def main() -> None:
    ...
    default_sort = get_param("screener_sort", "1d %")
    if default_sort not in sort_options:
        default_sort = "1d %"
    default_desc = bool(get_param("screener_desc", True))
    col_sort, col_order = st.columns([2, 1])
    selected_sort = col_sort.selectbox(
        "Sort by", list(sort_options.keys()), index=list(sort_options.keys()).index(default_sort)
    )
    if selected_sort != default_sort:
        set_param("screener_sort", selected_sort)
    descending = col_order.toggle("Descending", value=default_desc)
    if descending != default_desc:
        set_param("screener_desc", descending)

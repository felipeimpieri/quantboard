...
def main() -> None:
    ...
    if int(fast) != int(fast_default):
        set_param("fast", int(fast))
        fast_default = int(fast)

    if int(slow) != int(slow_default):
        set_param("slow", int(slow))
        slow_default = int(slow)

    if int(fee_bps) != int(fee_default):
        set_param("fee_bps", int(fee_bps))
        fee_default = int(fee_bps)

    if int(slip_bps) != int(slip_default):
        set_param("slippage_bps", int(slip_bps))
        slip_default = int(slip_bps)
